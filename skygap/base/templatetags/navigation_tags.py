...
from skygap.products.models import ProductPage, Product
...
def get_footer(context):
    return {
        'address': Address.objects.filter(live=True).first(),
        'email': Email.objects.filter(live=True).first(),
        'phone': Phone.objects.filter(live=True).first(),
        'copyright': Copyright.objects.filter(live=True).first(),
        'request': context['request'],
    }


@register.inclusion_tag('includes/logo.html', takes_context=True)
def get_logo(context):
    return {
        'logo': Logo.objects.filter(live=True).first(),
        'request': context['request'],
    }


@register.inclusion_tag('includes/product_types.html', takes_context=True)
def get_product_types(context):
    return {
